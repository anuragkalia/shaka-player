...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  onParseManifest_(has, wants) {
    ...
    const playerInterface = {
      ...
      // data inside segment can vary depending on mimeType and streamId
      onSegmentParsed: (mimeType, streamId, segment) => this.onSegmentParsed_(mimeType, streamId, segment),
  ...
  /**
   * Return a list of possible combinations available for the current period.
   * If the player has not loaded any content, this will return an empty list.
   */

  getVideoResolutions() {
    return shaka.Player.getVideoResolutionsFrom_(this.getVariantTracks());
  }

  restrictVideoQuality(heightRange, bandwidthRange) {
    const resolutions = this.getVideoResolutions();

    const someStreamAllowed = resolutions.some((resolution) => within_(heightRange, resolution.height) && within_(bandwidthRange, resolution.bandwidth / 1000));

    if (!someStreamAllowed) {
      shaka.log.alwaysWarn('No op - no stream allowed within given restrictions');
    } else {
      this.configure({
        restrictions: {
          minHeight: heightRange[0],
          maxHeight: heightRange[1],
          minBandwidth: bandwidthRange[0] * 1000,
          maxBandwidth: bandwidthRange[1] * 1000,
        },
      });
    }

    function within_(range, value) {
  ...
  /**
   * Get the video resolutions for a group of tracks for the selected language.
   *
   * @param {!Array.<?shaka.extern.Track>} tracks
   * @return {!Set.<number>}
   * @private
  ...
  static getVideoResolutionsFrom_(tracks) {
    let activeLanguage = '';
    const resolutionSetByLanguage = {[activeLanguage]: []};

    for (const track of tracks) {
      const language = track.language || 'und';
      resolutionSetByLanguage[language] = resolutionSetByLanguage[language] || new Map();
      shaka.Player.addToResolutionSet_(
          resolutionSetByLanguage[language],
          {
            width: track.width,
            height: track.height,
    ...
    return shaka.Player.formatResolutionSet_(resolutionSetByLanguage[activeLanguage]);
  }

  /**
   *
   * @param {!Map.<number, Set.<number>>} resolutionSet
   * @param {!Object.{{a: number, b: string, c}}} resolution
   */

  static addToResolutionSet_(resolutionSet, resolution) {
    if (!resolutionSet) {
      shaka.log.alwaysWarn('No resolution set to add resolution to!');
    } else {
      const {height, width, bandwidth} = resolution;
      if (!resolutionSet.has(height)) {
        resolutionSet.set(height, new Map());
      }
      const heightMap = resolutionSet.get(height);
      if (!heightMap.has(width)) {
        heightMap.set(width, new Set());
      }

      const widthSet = heightMap.get(width);
      widthSet.add(bandwidth);
    }
  }

  static formatResolutionSet_(resolutionSet) {
    if (!resolutionSet) {
      shaka.log.alwaysWarn('No resolution set to format!');
      return [];
    } else {
      const resolutions = [];

      const heights = Array.from(resolutionSet.keys()).sort(compareNumbersAndUndefined);
      for (const height of heights) {
        const heightMap = resolutionSet.get(height);
        const widths = Array.from(heightMap.keys()).sort(compareNumbersAndUndefined);
        for (const width of widths) {
          const widthSet = heightMap.get(width);
          const bandwidths = Array.from(widthSet).sort(compareNumbersAndUndefined);
          for (const bandwidth of bandwidths) {
            resolutions.push({height, width, bandwidth});
          }
        }
      }
    ...
    function compareNumbersAndUndefined(num1, num2) {
      if (num1 === undefined) {
        return 1;
...
shakaDemoMain.player_.addEventListener('variantchanged', () => {
  const history = shakaDemoMain.player_.getStats().switchHistory;
  if (history.length >= 2) {
    const currentEntry = history[history.length - 1];

    if (currentEntry.type === 'variant') {
      // find previous variant entry
      for (let i = history.length - 2; i >= 0; i--) {
        if (history[i].type === 'variant') {
          lastEntry = history[i];
          break;
        }
      }
      ...
      if (lastEntry) {
        const tracks = shakaDemoMain.player_.getVariantTracks();
        let lastTrack; let currentTrack;

        for (const track of tracks) {
          if (track.id === currentEntry.id) {
            currentTrack = track;
          }
          if (track.id === lastEntry.id) {
            lastTrack = track;
          }
        }

        if (lastTrack.language !== currentTrack.language) {
          const event = new shaka.util.FakeEvent('audiolanguagechanged', {
            lastAudioLanguage: lastTrack.language,
            currentAudioLanguage: currentTrack.language,
          });
          shakaDemoMain.player_.dispatchEvent(event);
        }
      }
    }
  }
...
shakaDemoMain.player_.addEventListener('audiolanguagechanged', (data) => {
  console.log(data);
});
