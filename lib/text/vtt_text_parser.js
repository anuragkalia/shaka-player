...
shaka.text.VttTextParser = class {
  ...
  /** @override **/
  getSegmentStartTime(data, time) {
    // Get WebVTT blocks from data
    const blocks = this.getBlocks_(data);
    ...
    if (segmentStart == null) {
      ...
      segmentStart = this.getTimestampMapOffset_(blocks, time);
    }
  ...
  parseMedia(data, time) {
    ...
    if (offset == null) {
      ...
      offset = this.getTimestampMapOffset_(blocks, time);
    }
   ...
   * @returns {number}
   * @private
   *   Offset calculated from 'X-TIMESTAMP-MAP', 0 otherwise
   */

  getTimestampMapOffset_(blocks, time) {
    ...
    return 0; /** no offset between subtitles and other streams */
  }
