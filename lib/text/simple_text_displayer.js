...
shaka.text.SimpleTextDisplayer = class {
   ...
   */
  append(cues) {
    const lessThanOrEqualTo = shaka.text.Cue.lessThanOrEqualTo;
    const convertToTextTrackCue =
        shaka.text.SimpleTextDisplayer.convertToTextTrackCue_;

    const lastCue = this.getLastTextTrackCue_();

    let cueIdx = 0;
    for (cueIdx = 0; cueIdx < cues.length; cueIdx++) {
      // This is okay because in WebVTT, all the subtitles are supposed to be
      // sorted on their startTime. So if we encounter anything less than what
      // is there currently in this.textTrack_, it means we can skip those cues.
      // This is used to detect duplicates across different WebVTT fragments
      // (allowed in HLS, not DASH).
      if (!lessThanOrEqualTo(cues[cueIdx], lastCue)) {
        break;
      }
    }
    // Convert cues.
    const textTrackCues = [];
    for (let i = cueIdx; i < cues.length; i++) {
      const cue = convertToTextTrackCue(cues[i]);
      if (cue) {
  ...
  /**
   * Either get last cue of textTrack_ or a dummy cue starting at -Infinity.
   * Format the last cue, if any, to shaka.text.Cue.
   *
   * @returns {shaka.text.Cue}
   * @private
   */

  getLastTextTrackCue_() {
    if (this.textTrack_.cues.length === 0) {
      return shaka.text.SimpleTextDisplayer.DummySmallestCue_;
    } else {
      const lastCue = this.textTrack_.cues[this.textTrack_.cues.length - 1];
      return new shaka.text.Cue(
          lastCue.startTime, lastCue.endTime, lastCue.text
      );
    }
  }
...
/**
 * @const {shaka.text.Cue}
 * @private
 */
shaka.text.SimpleTextDisplayer.DummySmallestCue_
  = new shaka.text.Cue(-Infinity, -Infinity, '');
